...
void ConvolutionFilter::resetOptions()
{
...
QImage ConvolutionFilter::apply(const QImage &image, const QRect& clipRect /*= QRect()*/ )
{
...
QImage ConvolutionFilter::convolve(const QImage &img, const QRect& clipRect)
{
	int top = 0;
	...
	for (int i = 0; i < m_kernels.count(); ++i) {
		...
		for (y = top; y < bottom; y++) {
			...
			for (x = left; x < right; x++) {
				line[x] = convolvePixelRGBA( normalizedImg, x, y, kernel.data(), 
								     kernel.rowCount(), kernel.columnCount(), divisor, bias);
			}
			int progress = (i * (bottom - top) + y) * 100.0 / ((bottom - top) * m_kernels.count());
					  ...
					  int divisor, int bias) const
{
	...
	for (i = 0; i < cols; i++) {
		for (j = 0; j < rows; j++) {
			...
			const QRgb* line = reinterpret_cast<const QRgb*>(img.scanLine(srcpix_y));
			QRgb rgb = line[srcpix_x];
