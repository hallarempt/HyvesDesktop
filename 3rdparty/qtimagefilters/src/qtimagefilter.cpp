...
QWidget* QtImageFilter::controlsWidget(QLabel *previewLabel, QImage originalPreviewImage) {
	m_previewLabel = previewLabel;
	m_originalPreviewImage = originalPreviewImage;
	handleControlValueChanged();
	return 0;
}

void QtImageFilter::setThreadedPreviewEnabled(bool enabled) {
	m_threadedPreviewEnabled = enabled;
}

bool QtImageFilter::threadedPreviewEnabled() const {
	return m_threadedPreviewEnabled;
}

void QtImageFilter::handleControlValueChanged() {

	if (m_previewLabel) {
		bool threadedPreview = m_threadedPreviewEnabled;
		QSlider *slider = qobject_cast<QSlider*>(sender());
		if (slider && slider->hasTracking()) {
			threadedPreview = false;
		}
		if (threadedPreview) {
			blockSignals(true); // make sure this apply doesnt send any signals now
			abortPreviewThread();
			m_currentPreviewFilterThread = new ImageFilterThread;
			m_currentPreviewFilterThread->setImageFilter(this);
			m_currentPreviewFilterThread->setImage(m_originalPreviewImage);
			connect(m_currentPreviewFilterThread, SIGNAL(finished()), this, SLOT(handlePreviewThreadFinished()));
			m_currentPreviewFilterThread->start();
		} else {
			m_previewLabel->setPixmap(QPixmap::fromImage(apply(m_originalPreviewImage)));
		}
	}
}

void QtImageFilter::handlePreviewThreadFinished() {

	if (!m_threadedPreviewEnabled) {
		return;
	}
	ImageFilterThread *filterThread = qobject_cast<ImageFilterThread*>(sender());
	if (filterThread == 0) {
		return;
	}
	if (filterThread->isAborted()) {
		delete filterThread;
		return;
	}
	if (m_previewLabel && m_currentPreviewFilterThread) {
		QImage resultImg = m_currentPreviewFilterThread->resultImage();
		if (!resultImg.isNull()) {
			m_previewLabel->setPixmap(QPixmap::fromImage(resultImg));
...
void QtImageFilter::abortPreviewThread() {
	if (m_threadedPreviewEnabled && m_currentPreviewFilterThread) {
		m_currentPreviewFilterThread->abort();
	}
	m_currentPreviewFilterThread = 0;
}

void QtImageFilter::cloudPreview() {
	if (m_previewLabel && !m_isPreviewBeingClouded) {
		QPixmap pixmap;
		if (m_previewLabel->pixmap()) {
			pixmap = m_previewLabel->pixmap()->copy();
		} else {
			pixmap = QPixmap::fromImage(m_originalPreviewImage);
		}
		...
		if (!pixmap.isNull()) {
			...
			m_previewLabel->setPixmap(newPixmap);
			m_isPreviewBeingClouded = true;
		}
	}
}

QtImageFilter *QtImageFilter::setCurrentSubFilter(const QString& filter, int m_progressAmount)
{
	m_currentSubFilterProgressAmount = m_progressAmount;
	m_progressTillLastSubFilter = m_totalProgress;
	m_currentSubFilter = QtImageFilterFactory::createImageFilter(filter);
	connect(m_currentSubFilter, SIGNAL(progressChanged(int)), this, SLOT(setSubFilterProgress(int)), Qt::DirectConnection);
	return m_currentSubFilter;
}

void QtImageFilter::setProgress(int progress) {
	
	if (m_totalProgress != progress) {
		m_totalProgress = progress;
		emit progressChanged(m_totalProgress);
	}
}


void QtImageFilter::setSubFilterProgress(int progress) {

	// arrives in a scale of 0-100: scale it to the appropriate fraction
	int subprogress = progress * ( m_currentSubFilterProgressAmount / 100.0);
	setProgress(m_progressTillLastSubFilter + subprogress);
}
		
void QtImageFilter::unsetCurrentSubFilter()
{
	//qDebug() << ">>> unset subfilter " << m_currentSubFilter->name();
	if (m_currentSubFilter) {
		m_currentSubFilter->disconnect(this);
		delete m_currentSubFilter;
		m_currentSubFilter = 0;
		setProgress(m_progressTillLastSubFilter + m_currentSubFilterProgressAmount);
	}
}
