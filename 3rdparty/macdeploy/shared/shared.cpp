...
bool copyFilePrintStatus(const QString &from, const QString &to)
{
    if (QFile::copy(from, to)) {
        QFile::Permissions permissions = QFile::permissions( to );
        ...
        QFile::setPermissions( to, permissions );
...
/**
 * @param appBundlePath path to the application bundle
 * @param path absolute or relative to cwd path to the plugin library
 * in the bundle.
 * @param info an info object with information about already deployed
 * libraries (to save extra copying)
 */
DeploymentInfo deployQtFrameworks( const QString &appBundlePath, const QString & path, const DeploymentInfo& info )
{
    ApplicationBundleInfo applicationBundle;
    applicationBundle.path = appBundlePath;
    applicationBundle.binaryPath = path;
    return deployQtFrameworks(getQtFrameworks(applicationBundle.binaryPath), applicationBundle.path, applicationBundle.binaryPath);
}
...
void deployPlugins(const ApplicationBundleInfo &appBundleInfo, const QString &pluginSourcePath, const QString pluginDestinationPath, DeploymentInfo deploymentInfo)
{
    ...
    foreach (QString pluginName, plugins) {
        ...
        //qDebug() << "deploy plugin dependencies:";
