...
cd build
cmake .. -G "NMake Makefiles" -DZLIB_INCLUDE_DIR=C:\ZLib\include -DZLIB_LIBRARY=C:\ZLib\lib\zdll.lib -DCMAKE_PREFIX_PATH=C:\OpenSSL;C:\Qt\4.5.0 -DCMAKE_BUILD_TYPE=RelWithDebInfo -DCMAKE_INSTALL_PREFIX=install
...
REM use svn revision number? But that's not available from here.
for /f "delims=/ tokens=1-3" %%a in ("%DATE:~3%") do (
        for /f "delims=:, tokens=1-4" %%m in ("%TIME: =0%") do (
                set FILENAME=winsymbolswin-%%a-%%b-%%c%%m-%%n-%%o-%%p.tgz
        )
)

tar -czf %FILENAME% symbols
